...
export const createStatistics = createAsyncThunk(
  ...
  async (formData, thunkAPI) => {
    try {
      const { data } = await axios.post("archives", formData, withToken(true));
...
export const deleteStatisticsById = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const { message } = await axios.delete(`archives/${id}`, withToken());
...
export const fetchStatisticsById = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const { data } = await axios.get(`archives/${id}`);
      return data;
    } catch (e) {
      const message = getErrorMessage(e);
      return thunkAPI.rejectWithValue(message);
